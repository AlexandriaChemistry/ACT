...
if __name__ == '__main__':
    ...
    if "darwin" in HOST:
        ...
        anaconda = '/opt/anaconda3/envs/ACT/lib'
    ...
    gmxdouble = ""
    FLAGS = ("-DMPIEXEC=%s -DMPIEXEC_NUMPROC_FLAG='-n' -DGMX_X11=OFF -DGMX_LOAD_PLUGINS=OFF -DBUILD_SHARED_LIBS=OFF -DGMX_OPENMP=OFF \
    -DGMX_MPI=ON -DGMX_GPU=OFF -DCMAKE_INSTALL_PREFIX=%s -DCMAKE_CXX_COMPILER=%s -DCMAKE_C_COMPILER=%s -DCMAKE_BUILD_TYPE=%s -DCMAKE_PREFIX_PATH='%s' \
    -DGMX_BUILD_MANUAL=OFF -DGMX_COMPACT_DOXYGEN=ON -DREGRESSIONTEST_DOWNLOAD=OFF -DGMX_DEFAULT_SUFFIX=OFF -DGMX_LIBXML2=ON -DGMX_EXTERNAL_BLAS=ON \
    -DGMX_EXTERNAL_LAPACK=ON %s" % ( mpirun, DEST, CXX, CC, args.build, PPATH, LBFLAGS ) )
    if args.cln:
        FLAGS += " -DGMX_CLN=ON"
    if not args.single:

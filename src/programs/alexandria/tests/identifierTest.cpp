...
#include "programs/alexandria/identifier.h"
...
TEST(IdentifierSimpleTest, ConstructorOne) {
    EXPECT_THROW(Identifier id({"C", "H"}, 0, CanSwap::Yes), gmx::InvalidInputError);
}
...
TEST(IdentifierSimpleTest, EqualNotSwappableAB) {
    Identifier a({"P", "H"}, CanSwap::No);
    Identifier b({"H", "P"}, CanSwap::No);
    bool equal = !(a < b) && !(b < a);
    EXPECT_FALSE((equal));
}

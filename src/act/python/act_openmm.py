...
class ActOpenMMSim:
    ...
    def argParser(self):
        ...
        defvdw = "WBH"
        parser.add_argument("-vdw", "--vdw", help="Van der Waals potential function. LJ for Lennard-Jones, WBH for modified Buckingham (Default), GWBH for generalized 4-parameter Buckingham"+defvdw, type=str, default=defvdw)
        defmDrude = "0.1"
        ...
        if args.debug:
            args.verbose = True
        # Do some checking
        if args.vdw != "LJ" and args.vdw != "WBH" and args.vdw != "GWBH":
        ...
        if args.xml_file and not os.path.exists(args.xml_file):
            sys.exit("Error: OpenMM force field file %s does not exist" % args.xml_file)
        elif args.act_file and not os.path.exists(args.act_file):
            sys.exit("Error: Alexandria force field file %s does not exist" % args.act_file)

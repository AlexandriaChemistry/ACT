...
class ActOpenMMSim:
    ...
    def argParser(self):
        ...
        atomic coordinates and a simulation parameter file.
        '''
        ...
        parser.add_argument("-alex", "--alexandria", help="Alexandria force field file, specify either this one or an OpenMM force field file", default=None)
    ...
    def add_direct_space_force(self): 
        ...
        self.add_force_group(self.nb_openmm, self.nb_openmm.getName())
        self.nb_correction = forces['CustomNonbondedForce']
#        self.nb_correction.setName("VDW_Correction")
#        self.add_force_group(self.nb_correction, self.nb_correction.getName())
        dforce = 'DrudeForce'
        ...
        if self.args.polarizable:
            ...
            self.add_force_group(pol_force, pol_force.getName())
            self.cores = []
            self.shells = []
            self.core_shell = []
        ...
        qqforce.setName("CoulombCorrection")
        ...
        self.add_force_group(qqforce, qqforce.getName())
        ...
        vdwforce.setName("LennardJonesCorrection")
...
#        print(f"cutoff d {cutoff_distance}")
        ...
        vdwforce.setUseLongRangeCorrection(self.nb_openmm.getUseDispersionCorrection())
        for index in range(self.nb_openmm.getNumParticles()):
        ...
        self.add_force_group(vdwforce, vdwforce.getName())
    ...
    def add_excl_correction(self):
        # Add vdW and electrostactics that have been excluded (this has to be done as the number
        # of exclusions is 3 for nonbonded interactions in OpenMM).
        # Those interactions are added using two CustomBondForce entries.
        vdw_force = openmm.CustomBondForce(self.vdw_expression)
        vdw_force.setName("VanderWaalsExclusionCorrection")
        ...
        qq_force.setName("CoulombExclusionCorrection")
        ...
        self.add_force_group(qq_force, qq_force.getName())
        self.system.addForce(qq_force)
        self.add_force_group(vdw_force, vdw_force.getName())
    ...
    def add_bonded_forces(self):
        ...
        for cb_force in self.system.getForces():
            if 'CustomBondForce' == cb_force.getName():
                ...
                self.add_force_group(cb_force, cb_force.getName())
    ...
    def print_force_settings(self):
        for force in self.system.getForces():
            print("----------------------------")
            print("%s Group: '%s' PBC: %s" % ( force.getName(), 
...
#            self.integrator.setMaxDrudeDistance(0.02*nanometer)
    ...
    def init_simulation(self):
        ...
        self.nb_correction.updateParametersInContext(self.simulation.context)

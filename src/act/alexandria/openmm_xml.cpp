                                 ...
                                 const std::map<std::string, int> &ffTypeMap)
{
    ...
    for(auto &it : allIT)
    {
        ...
        add_xml_char(gp, exml_names(xmlEntryOpenMM::NAME), "PotentialFunction");
        add_xml_char(gp, exml_names(xmlEntryOpenMM::DEFAULTVALUE), potentialToString(fs.potential()).c_str());

...
#define pnew (1-pold)
    ...
    if (gpin_)
    {
        ...
        *pool[pnew] = *pool[pold];
        ...
        individual->copyGenome(pool[pold]->genome(0));
        read = 1;
    ...
    if (read == 0)
    {
        pool[pold]->addGenome(individual->genome());
        pool[pnew]->addGenome(individual->genome());
        ...
        // from the ForceField structures and generated their own
        // starting values.
        for(auto &src : cr->middlemen())
        {
            ga::Genome genome;

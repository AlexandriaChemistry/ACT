...
the output of which is given in Table :numref:`tab-act-commands` .
...
contains a number of python scripts and utilities (Table :numref:`tab-scripts` ), some of which are needed for the quantum chemistry calculations as well (see Sections :ref:`sapt` and :ref:`qmdata`).
...
.. table:: Python utilities available in the ACT.
   ...
   |                     | for an example see Fig. :numref:`fig-ga-ftrain`         |
   +---------------------+---------------------------------------------------------+
...
ACT File formats
----------------
The ACT contains a number of specific file formats, in principle all of them generated by the ACT itself through scripts (Table :numref:`tab-scripts`) or *alexandria* (Table :numref:`tab-act-commands`). The files are listed below:
...
After this training instance, a correlation plot *EPOT.xvg* will be produced (Fig. :numref:`fig-epot`); you can use the::
...
You can inspect the different .xvg output files in the generated subdirectory *inds*. Fig. :numref:`fig-chi2` shows how the total deviation from SAPT data, you can recreate the figure using the::

    ./plot-chi2.sh    

script. Fig. :numref:`fig-zeta` shows how well the Gaussian distribution widths converge.
...
which will perform a normal mode analysis (Sec. :ref:`sec-nma`) and compute an infrared spectrum (Sec. :ref:`sec-irspectrum`) based on your new force field. This should yield a spectrum like the one in Fig. :numref:`fig-iract`.
...
.. figure:: ../images/ethanol-ir-act.pdf
   ...
   Simulated infrared vibrational spectrum for ethanol based on the force field derived in this tutorial (please compare to Fig. :numref:`fig-ethanol`).
...
and check the output structure, *after_em.pdb*. It should look like Fig. :numref:`fig-methdimer`.

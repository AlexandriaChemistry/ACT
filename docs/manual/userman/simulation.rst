...
Using ACT simulate
==================
The command *alexandria simulate* will perform a simulation in the gas phase, i.e. without periodic boundary conditions. This obviously limits the usefulness but the OpenMM software can be used for condensed-phase simulations using ACT force fields [Kriz2024a]_ [Spoel2025b]_ .
...
========================
The OpenMM~\cite{Eastman2023a} software in its native form is controlled from Python scripts. This allows users great control over the simulations. OpenMM allows to specify user-defined energy functions, which we use to implement both Gaussian-distributed charges~\cite{Ghahremanpour2018b,Walz2018a} and many Van der Waals potentials~\cite{Kriz2024a} (see Section~\ref{sec:energy}).

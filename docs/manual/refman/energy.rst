...
where :math:`\varepsilon_0` is the permittivity of vacuum, :math:`q_{i,j}` are the charges and :math:`\zeta_{i,j}` are the charge distribution widths (screening factors). Note that Eqn. :eq:`vcoulg` includes a relative dielectric constant :math:`\varepsilon_r` that can be used to parameterize a non-polarizable force field using charge-scaling~\cite{Kirby2019a}.

----------------------------------------
Coulomb Interaction using Slater charges
----------------------------------------
\label{sec:slater}
In addition, Slater distributed charges can be used as described in ref.~\cite{Ghahremanpour2018b}, from which the text below is an adapted version.
...
can then be written as~\cite{Rick1994a,Hentschke2004a}:
...
A number of methods~\cite{Ohrn2016a,Guseinov1970a} have been proposed to evaluate \eqnref{eqn:SlaterIntegral}. 
Hentschke gives an  analytical solution~\cite{Hentschke2004a}:
...
-------------------------------
It is good practice~\cite{Spoel2006a} to use the particle-mesh Ewald~\cite{Darden1993a,Essmann1995a} method to treat long-range electrostatics interactions in the condensed phase. In short, the method splits the Coulomb potential, either Eqn. :eq:`vcoul`, Eqn. :eq:`vcoulg` or something similar into a short and a long-range part as follows

.. math:: V_{coul}(r_{ij}) ~=~ V_{coul} \,{\rm erfc}(\alpha r_{ij}) +  V_{coul}\, {\rm erf}(\alpha r_{ij})
   :label: vcoullr

using the fact that the complementary error function {\rm erfc} equal 1 - {\rm erf}. The first term here is the short-range part, that is computed in real space, and the second term is the long-range part that is computed in Fourier (reciprocal) space by moving charges on a regular grid~\cite{Essmann1995a}.
...
Polarization
------------
Polarization can be treated explicitly as well in the ACT using the  core-shell model~\cite{Dick1958a,Jordan1995a} 
...
--------------------
\label{indcorr}
A term to add extra attraction between atoms was proposed in ref.~\citenum{McDaniel2013a}
...
-------------------------------------
The repulsion and dispersion interactions acting between atoms :math:`i` and :math:`j` can be described by a number of potentials. In a recent study on noble gases~\cite{Kriz2024a} we found that the 14-7 potential due to Halgren~\cite{Halgren1992a} was one of the most accurate ones:
...
where  :math:`\epsilon_{ij}` is the well depth at minimum, :math:`\gamma_{ij}` and :math:`\delta_{ij}` are dimensionless numbers that were originally shared for all elements. However, in our previous work~\cite{Kriz2024a}, we treated :math:`\gamma` and :math:`\delta` as free atom-specific parameters subject to optimization and combination rules.

Furthermore, the generalized 4-parameter Buckingham potential with an adjustable long-range attraction is implemented as~\cite{Werhahn2015a} 
...
Another potential that is supported is the buffered (Wang) Buckingham potential~\cite{Wang2013a}:
...
The buffered Buckingham potential mentioned above have been used to develop an accurate phase-transferable  model for alkali-halides~\cite{Walz2018a,Walz2019a,Walz2019b,Walz2019c,Walz2021a}
and the original Buckingham potential~\cite{Buckingham1938a} is implemented as
...
An alternative way of formulating the Buckingham potential~\cite{mason_transport_2004} is
...
Although mathematically identical to Eqn. :eq:`vbh` we have shown that the potentials behave differently when combination rules are applied~\cite{Kriz2024a}.


The well-known Tang-Toennies potential~\cite{Tang1984a,Tang2003a} containing five parameters is implemented according to:
...
This potential was used in our recent work on noble gases published where we investigated combination rules~\cite{Kriz2024a}.

In other works, the Tang-Toennies potential is extended with an additional parameter~\cite{Sheng2020a} giving the exponential functions a different decay length :math:`b`:
...
Van Vleet {\em et al.} described a more accurate formula for the Pauli repulsion~\cite{Vleet2016a} that was applied to derive a force field later~\cite{Vleet2018a}. In this model the exchange repulsion is given by the Slater-ISA formula
...
The Lennard-Jones 12-6 potential~\cite{Lennard-Jones1924b} is available as well:
...
To account for anisotropy in exchange, a correction can be implemented using a virtual site particle on the atom that has a :math:`\sigma`-hole interacting with other particles~\cite{Kriz2024b}: 
...
Multiple different combination rules can be used for parameters involved in van der Waals interactions, both within ACT and through the interface to OpenMM. For details, see the paper by K\v{r}{\'i}\v{z} {\em et al.}~\cite{Kriz2024a}.

----------------------------------
Treatment of long range dispersion
----------------------------------
The different Van der Waals potentials described above can be used with Lennard-Jones PME~\cite{Darden1993a,Wennberg2013a} if treated carefully. LJ-PME assumes a simple dispersion interaction given by
...
has to be minimized with respect to :math:`C_i` independently for each set of combination rules. In the Alexandria alkali-halide model we used the Wang-Buckingham potential (Eqn. :eq:`vwbh`) in conjunction with the combination rules due to Kong~\cite{Kong1973a} in which :math:`\sigma_{ij}` depends on all the :math:`\sigma`, :math:`\epsilon` and :math:`\gamma`. This makes it cumbersome to analytically derive integrals like Eqn. :eq:`dispintsum` for many combinations of potentials and combination rules.
...
----------------------------------------------
In a recent paper~\cite{Kriz2024a} we studied the effect of combination rules on potentials for noble gases and introduced two new combination rules (Eqn. :eq:`cr_volumetric` and Eqn. :eq:`cr_invsquare` ). Here is a brief summary, copied (with permission) from that paper.
...
The two simplest expressions that have historically been used as combination rules are the geometric~\cite{Berthelot1898a} and arithmetic~\cite{Lorentz1881a} averages, respectively:

.. math:: X_{12} = \sqrt{x_{1} x_{2}}
   :label: cr_geometric

%was used for all three parameters :math:`\epsilon`, :math:`\gamma`, :math:`\sigma` in "geometric" rules. It is also used for :math:`\epsilon` in "arithmetic" rules and for :math:`\sigma` in "Kong-Masen" rules \cite{}.
...
Hogervorst introduced a set of combination rules for 12-6 Lennard-Jones and exp-6, modified Buckingham, potential (Eqn. :eq:`MBH`)~\cite{Hogervorst1971a}. He proposed using Eqn. :eq:`cr_hogervorst_epsilon` for :math:`\epsilon` for both potential forms. For the :math:`\sigma` of 12-6 potential (Eqn. :eq:`12_6`), the
...
Yang {\em et al.}\cite{Yang2018_combination} introduced an expression  for the Morse potential~\cite{Morse1929a}, using Eqn. :eq:`cr_hogervorst_epsilon` for :math:`\epsilon`, while eqn. :eq:`cr_yang` is used for :math:`\sigma` and  :math:`\gamma`. 

.. math:: X_{12} = \frac{x_{1} x_{2}(x_1 + x_2)}{x_{1}^2 + x_{2}^2}
   :label: cr_yang

The expression for :math:`\gamma`  proposed by Mason~\cite{Mason1955a} for the exp-6 potential has the form
...
Waldman and Hagler\cite{Waldman1993a} introduced expressions \ref{f8} for :math:`\epsilon` and  \ref{f6} for :math:`\sigma`
...
Although Eqn. :ref:`f6` was devised for :math:`\sigma` we have evaluated it for other parameters here as well, hence the notation with :math:`X`. Qi and coworkers advocated the use of buffered 14-7 Lennard-Jones potential (Eqn. :eq:`14_7`) due to Halgren~\cite{Halgren1992a}, alongside  combination expressions \ref{f8} for :math:`\epsilon` and  \ref{f7} for :math:`\sigma`: \cite{qi_general_2016} 
%\red{In Alexandria, again, Eqn. :ref:`f2` is used :math:`\gamma`}

.. math:: X_{12} = \frac{x_1^3+x_2^3}{x_1^2+x_2^2}
   :label: f7

A further relation, the harmonic mean rule, was proposed by Halgren~\cite{Halgren1992a}:

.. math:: X_{12} =  \frac{4 x_1 x_2}{(x_1^{1/2}+x_2^{1/2})^2}
   :label: cr_harmonic_mean


Finally, we introduce two new combination rules that we have not seen published previously. Since the :math:`\sigma` in most potentials can be interpreted as a Van der Waals radius, we introduced a relation averaging  third powers, corresponding to an atomic volume~\cite{Kriz2024a}:
...
In our previous work on alkali halides~\cite{Walz2018a} we used combination rules according to eqn. :eq:`cr_sigma5` for :math:`\sigma`, eqn. :eq:`cr_hogervorst_epsilon` for :math:`\epsilon` and eqn. :eq:`cr_arithmetic` for :math:`\gamma` with the Wang-Buckingham potential (Eqn. :eq:`vwbh`).

A number of these combination rules can be written using the generalized mean equation~\cite{Hohm2025a}

.. math:: M_p(x_1,x_2,...,x_N) = \left(\frac{1}{N}\sum_{i=1}^N x_i^p\right)^{1/p}
   :label: cr_genmean

for :math:`p \ne 0`. For :math:`p = 0`, Eqn. :eq:`cr_genmean` turns into the geometric rule. Table :ref:`tab-genmean` lists other well-known combination rules and their respective exponent :math:`p`. Hohm also describes combinations of the generalized mean and other similar expressions, but the most important observation he made was that the exponent :math:`p` can be varied at will~\cite{Hohm2025a}. In the ACT it is possible to train this parameter along with the Van der Waals parameters.
...
Morse potential
------------------
A Morse potential~\cite{Morse1929a} can be used, with one addition:
...
Wei-Hua potential
------------------
In a very recent study we found the potential due to Hua~\cite{Hua1990a,Hua1990b} to be the best compromise between accuracy of the vibrational frequencies and computational cost~\cite{Maaren2025a}. 
It is given by:
...
with k :sub:`lin` the force constant~\cite{Spoel2020a}. 
...
where :math:`c_n` are constants and the torsion angle is defined as above. The constant :math:`\pi` is added to be compatible with the Ryckaert-Bellemans potential~\cite{Ryckaert1975a} that is implemented in simulation codes like GROMACS~\cite{Spoel2005a} and OpenMM~\cite{Eastman2010a}.
...
A virtual site is an extra point, located at a defined position in a molecule. A variety of virtual site options is currently implemented within the ACT framework:
  ...
  * a virtual site on top of an atom (VSITE1)~\cite{Spoel2025a_PG}
  * a virtual sites along the bond (VSITE2) for the description of anisotropic charge distribution and exchange~\cite{Kriz2024b} such as encountered in :math:`\sigma` holes
  * a virtual site on the bisector of a angle, like in the TIP4P water model~\cite{Jorgensen1983a} (VSITE3S) or in alcohol (VSITE3)
  * off-plane virtual sites for modeling lone-pairs in sp:math:`^3` hybridized compounds, such as water (VSITE3SOUT, symmetric) or asymmetric for compounds like alcohols~\cite{Mahoney2000a,Kriz2024b}.

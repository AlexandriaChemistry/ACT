...
Finally, :math:`\Lambda` adds a penalty to keep (some of the) parameters within bounds. The first term can be expanded into a sum, since multiple observable can be trained at the same time.
